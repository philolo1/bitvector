    ...
    for (node_id_t i = 0; i < n; i++) {
      ...
      if (visited[i] == false) {
        ...
        while (!queue.empty()) {
          ...
          for (node_id_t i = 0; i < get_number_in_neighbours(current); i++) {
            Edge in_edge = get_in_neighbour(current, i);
            if (!visited[in_edge.target]) {
              visited[in_edge.target] = true;
              queue.push_back(in_edge.target);
            }
          }
        }
...
int main(int argc, char **argv) {
 ...
 if (argc != 4) {
    ...
    std::cout << "Usage: " << argv[0]
              << " [nocomp|lz4|lzhc|simd|stl] [input_graph] [output_file]" << std::endl;
