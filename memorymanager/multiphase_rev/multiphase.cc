...
// code is inspidred by http://stackoverflow.com/questions/669438/how-to-get-memory-usage-at-run-time-in-c
...
void testNormal(int allocPow, int iterationPow, int chunkPow, int deleteCond) {
  ...
  double vm, rss;
  double startVSS = 0.0, startRSS = 0.0, midVSS = 0.0, midRSS = 0.0, endVSS =0.0, endRSS = 0.0;

  int allocSize = 1 << allocPow;
  int chunkSize = 1 << chunkPow;
  int iterations = 1 << iterationPow;

  for (int numberOfExperiments = 0; numberOfExperiments < NUM_TRIALS; numberOfExperiments++){
  ...
  cout << "NORMAL\t" << allocPow << "\t" << iterationPow << "\t" << chunkPow << "\t" << startVSS/NUM_TRIALS << "\t" << startRSS/NUM_TRIALS  << "\t" << midVSS/NUM_TRIALS << "\t" << midRSS/NUM_TRIALS << "\t" << endVSS/NUM_TRIALS << "\t" << endRSS/NUM_TRIALS;
  timer.printTime("");
...
void testSimple(int allocPow, int iterationPow, int chunkPow, int deleteCond) {
  ...
  cout << "SIMPLE\t" << allocPow << "\t" << iterationPow << "\t" << chunkPow << "\t" << startVSS/NUM_TRIALS << "\t" << startRSS/NUM_TRIALS << "\t"  << midVSS/NUM_TRIALS << "\t" << midRSS/NUM_TRIALS << "\t" << endVSS/NUM_TRIALS << "\t" << endRSS/NUM_TRIALS;
  timer.printTime("");
