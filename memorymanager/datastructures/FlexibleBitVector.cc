...
public:
  pair<int, int> getMemorySize() {
    return  make_pair(sizeof(this), tree->getMemorySize());
  }
  ...
  FlexibleBitVector(int numberOfBlocks, int chunkSize, int S) {
    ...
    MemoryProvider<Compressor, MemManager, MemPointer>
 provider;
    ...
    // nothing should be bigger than
       cache = provider.getCache(maxSize, mem);

       buffer = cache->flexBuffer1;
       outBuffer = cache->flexBuffer2;

      
      // new FlexibleCache<CACHE_SIZE, Compressor, MemManager, MemPointer>(maxSize, mem);
  ...
  void addBlock(void *block) {
    ...
    cache->push((char *)block, node, bitElementCounter, tree);
  ...
  uint32_t accessUint32_t(int pos) {
    ...
    pair<FlexibleAVLNode *, int> res = cache->getObjectByIndex(pos * INT_BIT, tree);
    ...
    outBuffer = cache->get(node, res.second, tree);
  ...
  void deleteUint32_tSimple(int pos, FlexibleAVLNode *node, int startIndex) {
    ...
    outBuffer = cache->get(node, startIndex, tree);
  ...
  void deleteUint32_t(int pos) {
    ...
    pair<FlexibleAVLNode *, int> res = cache->getObjectByIndex(pos, tree);
    if (res.first == nullptr) {
      res = tree->find(pos);
    }

    cache->updateStartPos(pos, -1 * (int)INT_BIT, tree);
    ...
    if (node->len - sizeof(uint32_t) > chunkSize / 2) {
      ...
      outBuffer = cache->get(leftNode.first, leftNode.second, tree);
      ...
      if (lenLeft + lenRight <= chunkSize + chunkSize / 2) {
        ...
        int startPos = leftNode.second + CHAR_BIT * newLen1;
        FlexibleAVLNode *newNode =
            tree->insert(startPos, nullptr, sumOnes - oneCount, newLen2);

        cache->push(outBuffer + newLen1, newNode, startPos, tree);
  ...
  void insertBlock(int pos, char *data, int dataLen) {
    ...
    bool isLastElement = pos == bitElementCounter;

    pair<FlexibleAVLNode *, int> res;

    if (isLastElement) {
      if (pos > 0) {
        res = cache->getObjectByIndex(pos - INT_BIT, tree);
        if (res.first == nullptr) {
          res = tree->find(pos - INT_BIT);
        }
      } else {
        ...
        cache->push(data, node, res.second, tree);

        bitElementCounter += dataLen * INT_BIT;
        return;
      }
    } else {
      res = cache->getObjectByIndex(pos, tree);
      if (res.first == nullptr) {
        res = tree->find(pos);
      }
    }
    ...
    cache->updateStartPos(pos, dataLen * INT_BIT, tree);
    ...
    if (node->len + (dataLen * sizeof(uint32_t)) < 2 * chunkSize) {
      ...
      // only update if it is not the last elemeent
      ...
      int sumSecondHalf = node->numOnes + addNum - sumFirstHalf;
      ...
      cache->push(outBuffer + node->len, newNode, start, tree);
    }
  ...
  int select(int num) {
    ...
    outBuffer = cache->get(node, -1, tree);
  ...
  int rank(int pos) {
    ...
    // TODO set the right value here for caching
    outBuffer = cache->get(node, -1, tree);
